...
export const getBills = async (req, res) => {
    try {
        const bill = await Bill.find({}).populate("userId").populate("cartId")
...
// LẤY ĐƠN HÀNG THEO ID BILL 
export const getIdBill = async (req, res) => {
    try {
...
export const checkBill = async (req, res) => {
    ...
    try {
        ...
        const bill = await Bill.findOne({ userId })
            .populate("cartId")
            .populate("userId")
            // .populate("cartId.products.productId")
        if (!bill) {
